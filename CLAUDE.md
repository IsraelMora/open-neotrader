...
Risk is **safe by default** (opt-out, not opt-in): `TradeIntentService` enforces a kernel floor regardless of active plugins — a max-drawdown halt and a per-trade size ceiling (`max_position_pct`), on both the autonomous and human-approval paths. `exit`/`hold` always bypass every gate (a position must always be closeable). Plugins can only add MORE restriction on top. Real execution is additionally gated: `_effectiveMode` returns `real` only when `policy.real` + `broker_plugin_id` + the applied strategy has a recent `ROBUSTO` walk-forward verdict (`execution.walk_forward_max_age_days`); any failure demotes to paper (never blocks it). Persisting a walk-forward verdict requires TOTP.

**Real-money accounting (wired).** Real orders are tracked end-to-end, never fabricated: `_executeReal` submits via `RealOrderService` (idempotent per intent via a `client_order_id` + a partial-unique index; the row is committed `pending_submit` before the broker POST), the intent stays `real_pending` until `RealBrokerReconciliationService` confirms the broker fill and, in ONE `$transaction`, sets `RealOrder` + `TradeIntent` together (partial fills preserved). Reconciliation polls (fast-poll after submit + a steady tick that also recovers in-flight rows), syncs `RealPosition`/`RealNavSnapshot` from the broker, and raises `BROKER_DRIFT`/`RECONCILIATION_HALTED` alerts. In real mode the kernel gates read the **real** account (`RealNavSnapshot` equity/HWM/`buying_power`, `RealPosition` count) and **fail closed** — a missing/stale (`execution.real_state_max_age_ms`)/non-finite real snapshot blocks new real entries (never exits). A global kill-switch (`real_execution.halted`) auto-trips on drift / reconciliation circuit-breaker / repeated submit failures and clears only via a TOTP endpoint.

**Paper mode is the supported default and the recommended path.** Real-money mode is now functionally complete but should still be enabled deliberately and evaluated carefully before funding. Residual known limitations: fills are confirmed by polling (no streaming websocket yet); partial-fill remainder auto-cancel-on-timeout is not implemented; `RealPosition` is keyed by symbol only (single active `broker_plugin_id`); the paper `NavSnapshot.getHistory` oldest-N ordering bug is unrelated and untouched. Start in paper; do not run real money you can't afford to lose.
