/**
 * ml-signal-record.boot.spec.ts — Task 5.1 TDD RED → 5.2 GREEN
 *
 * ml-feature-extractor-s1: No-circular-DI boot test.
 *
 * Verifies that MlSignalRecordModule + AgentsModule + SnapshotModule compile
 * together in a real NestJS Test.createTestingModule without any circular-dependency
 * warning. MlSignalRecordModule is a leaf (PrismaModule only), so:
 *   AgentsModule → MlSignalRecordModule (leaf)
 *   SnapshotModule → MlSignalRecordModule (leaf)
 * are both leaf edges — no cycle possible.
 *
 * Pattern mirrors ltm-cycle-integration.boot.spec.ts (F6-s2 PR2).
 */
...
import { MlSignalRecordModule } from './ml-signal-record.module';
...
const mlSignalRecordMock = {
  recordSignals: jest.fn().mockResolvedValue(undefined),
  updateOutcomeAggregate: jest.fn().mockResolvedValue(undefined),
...
describe('ml-feature-extractor-s1 — MlSignalRecord+Agents+Snapshot module boot (no circular deps)', () => {
  ...
  beforeAll(async () => {
    ...
    moduleRef = await Test.createTestingModule({
      ...
      .useValue(ltmMock)
      .overrideProvider(MlSignalRecordService)
      .useValue(mlSignalRecordMock)
      .compile();
  });
  ...
  it('MlSignalRecordService is resolvable', () => {
    const svc = moduleRef.get(MlSignalRecordService);
